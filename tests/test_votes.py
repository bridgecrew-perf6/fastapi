...
def test_vote_post_not_exist(authorized_client, test_posts, dir):
    res = authorized_client.post("/vote/", json={"post_id": 98988888, "dir": dir})
    assert res.status_code == 404


@pytest.mark.parametrize("dir", [(0), (1)])
def test_vote_unauthorized_user(client, test_posts, dir):
    res = client.post("/vote/", json={"post_id": test_posts[0].id, "dir": dir})
